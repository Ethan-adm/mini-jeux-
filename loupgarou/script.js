...
// Variables locales pour le Joueur (Vote Village)
...
document.getElementById('join-btn').addEventListener('click', () => {
    ...
    onValue(ref(db, `rooms/${room}/players`), (snap) => {
        ...
        if (!currentPlayers[myPlayerId] && document.getElementById('lobby-screen').classList.contains('active')) {
            alert("Vous avez été expulsé du salon."); location.reload();
        }
    ...
    onValue(ref(db, `rooms/${room}/votes`), (snap) => {
        ...
        if (amINarrator && gameState.phase === "vote_village") renderNarratorUI(); // Maj en direct des votes pour le narrateur
    });

    // MOTEUR D'ETAT GLOBAL (GameState)
    onValue(ref(db, `rooms/${room}/gameState`), (snap) => {
        gameState = snap.val() || {};
        if (!gameState.phase) return;

        // Passage de Lobby à Jeu
...
function updateLobby() {
    ...
    for (let id in currentPlayers) {
        ...
        li.textContent = prefix + p.name + (id === myPlayerId ? " (Toi)" : "");
        ...
        if(narratorSelect) {
            ...
            if (id === myPlayerId) opt.selected = true; // Par défaut le chef est narrateur
...
// 3. LANCER LE JEU (Admin)
document.getElementById('start-game-btn').addEventListener('click', () => {
    let narratorId = document.getElementById('narrator-id').value;
    const allIds = Object.keys(currentPlayers);
    let playersToAssign = allIds.filter(id => id !== narratorId);

    if (playersToAssign.length === 0) return alert("Il faut des joueurs en plus du narrateur !");
    ...
    updates[`rooms/${currentRoom}/gameState`] = {
        phase: "nuit_debut",
        cupidonDone: false, lovers: [], nightLog: "",
...
// ==========================================
// INTERFACE JOUEUR (Ce que voient les autres)
// ==========================================
function renderPlayerUI() {
    ...
    if (["nuit_debut", "cupidon", "voyante", "loups", "sorciere", "nuit_fin"].includes(gameState.phase)) {
        nightCover.style.display = "flex";
    } else {
        nightCover.style.display = "none";
    }

    if (gameState.phase === "jour" || gameState.phase === "resultat_village") {
        dayAnnounce.style.display = "block";
        dayAnnounce.textContent = buildDeathMessage(gameState.deathsToAnnounce);
    }

    if (gameState.phase === "vote_village" && !currentPlayers[myPlayerId].isDead) {
        voteZone.style.display = "block";
        renderPlayerVoteButtons();
    }
}

function buildDeathMessage(deaths) {
    if (!deaths || deaths.length === 0 || deaths.includes("none")) return "Le village se réveille. Personne n'est mort cette nuit !";
    let t = "Tragédie ! Mort(s) : ";
    deaths.forEach(id => {
...
// Fonction de vote (côté joueur)
function renderPlayerVoteButtons() {
    const list = document.getElementById('vote-list');
    list.innerHTML = ""; selectedTargetIds = [];
    ...
    skipBtn.onclick = () => selectTargetPlayer("skip", skipBtn); list.appendChild(skipBtn);
    ...
    for (let id in currentPlayers) {
        ...
        btn.onclick = () => selectTargetPlayer(id, btn); list.appendChild(btn);
    }
    ...
    document.getElementById('btn-confirm-action').onclick = () => {
        ...
        document.getElementById('my-vote-status').textContent = "Vote envoyé au Narrateur.";
    };
}
function selectTargetPlayer(id, btnElement) {
...
// ==========================================
// INTERFACE NARRATEUR (Le Cerveau du Jeu)
// ==========================================
function renderNarratorPlayerList() {
    ...
    for (let id in currentPlayers) {
        ...
        const p = currentPlayers[id];
        const li = document.createElement('li');
        li.style.padding = "8px"; li.style.margin = "4px 0";
        ...
        else li.innerHTML = `<span>${p.name} <span style="color:#e94560; font-size: 0.8em;">[${p.role}]</span></span> <span style="color:#28a745">En vie</span>`;
        list.appendChild(li);
    }
...
function generateSelectAlive(id, label, defaultOpt = "Personne") {
    ...
    for (let pid in currentPlayers) {
        if (!currentPlayers[pid].isDead && pid !== myPlayerId) html += `<option value="${pid}">${currentPlayers[pid].name} (${currentPlayers[pid].role})</option>`;
    }
    return html + `</select>`;
}
...
function renderNarratorUI() {
    ...
    nextBtn.onclick = handleNarratorNext; // Bind function
    ...
    switch(gameState.phase) {
        case "nuit_debut":
            scriptBox.textContent = `"La nuit tombe sur le village. Tout le monde ferme les yeux et s'endort."`;
            actionBox.innerHTML = "<i>Attendez que tout le monde ait les yeux fermés.</i>";
            break;
        case "cupidon":
            scriptBox.textContent = `"Cupidon se réveille. Il désigne deux personnes qui tomberont follement amoureuses."`;
            actionBox.innerHTML = generateSelectAlive("cup-1", "Amoureux 1 :") + generateSelectAlive("cup-2", "Amoureux 2 :");
            break;
        case "voyante":
            scriptBox.textContent = `"La voyante se réveille. Elle me désigne un joueur pour découvrir son véritable rôle."`;
            actionBox.innerHTML = generateSelectAlive("voy-target", "Joueur inspecté :");
            setTimeout(() => {
                document.getElementById('voy-target').onchange = (e) => {
                    let v = e.target.value;
                    if(v !== "none") actionBox.innerHTML += `<div style="margin-top:10px; padding:10px; background:#e94560; border-radius:5px;">Rôle à montrer (en silence) : <b>${currentPlayers[v].role}</b></div>`;
                };
            }, 50);
            break;
        case "loups":
            scriptBox.textContent = `"Les loups-garous se réveillent. Ils se reconnaissent et désignent une victime."`;
            actionBox.innerHTML = generateSelectAlive("wolf-target", "Victime des loups :");
            break;
        case "sorciere":
            scriptBox.textContent = `"La sorcière se réveille. Je lui montre la victime des loups. Veut-elle utiliser sa potion de vie ou de mort ?"`;
            ...
            if(!gameState.witchLifeUsed && wVictimName !== "Personne") {
                html += `<label style="display:block; margin-bottom:10px;"><input type="checkbox" id="witch-revive"> 🧪 Ressusciter ${wVictimName}</label>`;
            } else { html += `<p style="color:#888; font-size:0.9em;">(Potion de vie utilisée ou inutile)</p>`; }

            if(!gameState.witchDeathUsed) {
                html += generateSelectAlive("witch-kill", "☠️ Tuer avec la potion de mort :");
            } else { html += `<p style="color:#888; font-size:0.9em;">(Potion de mort déjà utilisée)</p>`; }
            
            actionBox.innerHTML = html;
            break;
        case "nuit_fin":
            scriptBox.textContent = `(Calcul des morts en cours... Cliquez sur suivant)`;
            actionBox.innerHTML = "Résolution de la nuit...";
            break;
        case "jour":
            scriptBox.textContent = `"Le soleil se lève sur le village. Tout le monde se réveille... et ouvre les yeux."`;
            actionBox.innerHTML = `<p>${buildDeathMessage(gameState.deathsToAnnounce)}</p>`;
            break;
        case "chasseur":
            scriptBox.textContent = `"Le chasseur est mort ! Dans son dernier souffle, il riposte et choisit une cible."`;
            actionBox.innerHTML = generateSelectAlive("hunter-target", "Victime du chasseur :");
            break;
        case "vote_village":
            scriptBox.textContent = `"Le village débat. C'est l'heure de voter sur vos téléphones pour éliminer un suspect."`;
            
            // Affichage des votes en direct
            ...
            voteHtml += `</ul><p style="font-size:0.8em; color:#aaa;">(Attendez que tous votent avant de clôturer)</p>`;
            actionBox.innerHTML = voteHtml;
            
            revoteBtn.style.display = "block";
            revoteBtn.onclick = () => update(ref(db, `rooms/${currentRoom}/votes`), null);
            nextBtn.textContent = "⚖️ Clôturer le Vote et Tuer";
            break;
        case "resultat_village":
            scriptBox.textContent = `Le village a parlé. (Annoncez le mort).`;
            actionBox.innerHTML = `<p>${buildDeathMessage(gameState.deathsToAnnounce)}</p>`;
            break;
    }
}

// LOGIQUE D'ENCHAINEMENT DU NARRATEUR
...
function handleNarratorNext() {
    ...
    let updates = {};
    let next = "nuit_debut";
    let newLog = gameState.nightLog || "";

    // FONCTIONS UTILITAIRES
    let addDeath = (id, cause) => {
    ...
    if (p === "nuit_debut") {
        ...
        if (roleExists("🏹 Cupidon") && !gameState.cupidonDone) next = "cupidon";
        else if (roleExists("👁️ Voyante")) next = "voyante";
        else next = "loups";
    }
    else if (p === "cupidon") {
        let c1 = document.getElementById('cup-1').value; let c2 = document.getElementById('cup-2').value;
        if(c1 !== "none" && c2 !== "none") {
            updates[`rooms/${currentRoom}/gameState/lovers`] = [c1, c2];
            updates[`rooms/${currentRoom}/gameState/cupidonDone`] = true;
            newLog += `🏹 Cupidon a lié ${currentPlayers[c1].name} et ${currentPlayers[c2].name}.`;
        }
        if (roleExists("👁️ Voyante")) next = "voyante"; else next = "loups";
    }
    else if (p === "voyante") {
        let v = document.getElementById('voy-target')?.value;
        if(v && v!=="none") newLog += `\n👁️ Voyante a vu le rôle de ${currentPlayers[v].name}.`;
        next = "loups";
    }
    else if (p === "loups") {
        ...
        if (roleExists("🧙‍♀️ Sorcière")) next = "sorciere"; else next = "nuit_fin";
    }
    else if (p === "sorciere") {
        ...
        if(revive) {
            updates[`rooms/${currentRoom}/gameState/witchLifeUsed`] = true;
            updates[`rooms/${currentRoom}/gameState/wolfTarget`] = "none"; // Annule l'attaque
            newLog += `\n🧪 Sorcière a sauvé la victime.`;
        }
        if(kill !== "none") {
            updates[`rooms/${currentRoom}/gameState/witchDeathUsed`] = true;
            updates[`rooms/${currentRoom}/gameState/witchTarget`] = kill;
            newLog += `\n☠️ Sorcière a tué ${currentPlayers[kill].name}.`;
        } else { updates[`rooms/${currentRoom}/gameState/witchTarget`] = "none"; }
        next = "nuit_fin";
    }
    else if (p === "nuit_fin") {
        // Appliquer les morts de la nuit
        ...
        if (deaths.some(id => currentPlayers[id] && currentPlayers[id].role === "🔫 Chasseur")) next = "chasseur";
        else if (p === "jour") next = "vote_village";
        else next = "nuit_debut"; // Fin du jour
    }
    else if (p === "vote_village") {
        // Calculer le plus voté
        let counts = {};
        for(let v in currentVotes) { if(currentVotes[v]!=="skip") counts[currentVotes[v]] = (counts[currentVotes[v]]||0)+1; }
        let highest = null, max = 0;
        for(let t in counts) { if(counts[t]>max){ max=counts[t]; highest=t; } }
        
        updates[`rooms/${currentRoom}/votes`] = null; // reset
        updates[`rooms/${currentRoom}/gameState/deathsToAnnounce`] = [];
        if(highest) { addDeath(highest); newLog += `\n⚖️ Le village a exécuté ${currentPlayers[highest].name}.`; }
        next = "resultat_village";
    }
    else if (p === "chasseur") {
        let t = document.getElementById('hunter-target').value;
        updates[`rooms/${currentRoom}/gameState/deathsToAnnounce`] = [];
        if(t!=="none") { addDeath(t); newLog += `\n🔫 Chasseur a tué ${currentPlayers[t].name}.`; }
        next = "resultat_village"; // On redirige vers resultat_village pour afficher ce mort, puis la boucle repart
    }

    // CHECK VICTOIRE
    // Appliquons temporairement les morts pour le check
    ...
    if (winMsg && p !== "nuit_debut" && p !== "cupidon" && p !== "voyante" && p !== "loups" && p !== "sorciere") {
